...
| 术语      | 解释                                            |
| --        | --                                              |
| GNS-J1900 | 指的是固高以ATOM J1900为主控芯片方案的GNS控制器 |
| GNS-N150  | 指的是固高以ATOM N150为主控芯片方案的GNS控制器  |

# 背景

本文档用于说明在 BIOS/UEFI 不同的引导架构下，GNS-N150 平台与基于传统 BIOS 引导的 GNS-J1900 平台在系统烧录、备份及还原流程上的差异，并阐述对应方案的设计动机与实现原则。
...
进行 GNS 系统的烧录与备份操作前，需要准备并确认以下硬件与环境条件：

* GNS-J1900/GNS-N150 控制器 ×1
* USB 键盘 ×1
* 带 DP 接口的显示器 ×1
* DP 视频连接线 ×1
* U 盘 ×1（需提前格式化为 FAT/FAT32 文件系统）
* GNS-J1900/GNS-N150 专用 24V 电源 ×1
...
1. 确认 Windows PC 已配置为可**自动连接**远程服务器的 SAMBA 共享目录：
   `\\192.168.201.73\share_mbr`。

2. 将需要进行递增备份打包的 **ISO 基础镜像文件** 上传至上述共享目录。

3. 将需要进行递增备份打包的 **CODESYS 压缩包** 上传至同一共享目录，
   压缩包格式支持 `.tar.gz`、`.zip`、`.rar`。

4. 在共享目录中创建一个**无后缀的文本文件** `VERSION`，
   文件内容为更新后镜像对应的版本号，版本号采用 **8 位数字格式**，例如 `20220101`。

5. 在共享目录中创建一个**无后缀的空文件** `I_WANT_TO_UPDATE_CODESYS`，
   该文件作为触发自动打包流程的标志。

6. 当上述文件准备完成后，服务器后台服务将被自动触发，
   开始执行新的镜像递增打包流程。

![](./pictures/auto-package-workflow.png){width=100%}

# 系统备份流程（GNS-J1900）

系统备份流程如下图所示：

![](pictures/j1900-backup-workflow.png){width=100%}

注意：

1. 由于历史设计原因，GNS J1900 的备份镜像与系统安装镜像相互独立，分别维护。
   备份镜像存放路径为： `\\192.168.201.13\共享文件夹\linux镜像文件\GNS-Backup-Image`

2. 制作备份U盘时，U盘容量必须大于等于 4GB。

3. 制作备份U盘必须使用 `Universal-USB-Installer` 工具。

4. 在重启并进入备份U盘之前，必须运行检查脚本 `googol_release_check.sh`。
   该脚本用于清理相关标志位，并检查关键服务是否处于预期状态。

5. 备份生成的文件系统数据目录，可使用自动化打包工具处理，也可以使用 ISO 图形工具手动修改。
   在可以访问服务器的情况下，建议优先使用自动化打包工具。具体步骤如下：

   a. 访问共享目录：`\\192.168.201.73\share_mbr  `
   b. 在目录中创建标志文件 `VERSION`，内容为版本号（11位字符串），例如： `20240601R01 `
   c. 将最新备份镜像文件复制到该目录  
   d. 将U盘中的文件系统数据目录复制到该目录  
   e. 创建空文件 `I_WANT_TO_UPDATE_ROOTFS`  
   f. 等待后台程序自动生成新的 `ISO` 镜像文件
